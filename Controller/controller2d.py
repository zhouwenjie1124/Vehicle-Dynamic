...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            i_term = ki * self.vars.error_integral

            # D term
            d_term = kd * error - self.vars.error_previous * dt
            
            # PID output
            pid_output = p_term + i_term + d_term

            # use sigmoid function constrain the output from 0 to 1
            throttle_output = 1 / (1 + np.exp(-pid_output))
            ...
            crosstrack_error = np.min(np.sum((current_xy - np.array(waypoints)[:, :2])**2, axis=1)) # find the closest point
            ...
            if yaw_path2ct > 0:
                ...
                crosstrack_error = - abs(crosstrack_error)
            print(crosstrack_error, yaw_diff)

            # 3. control law 
            ke = 0.3
            steer_expect = yaw_diff + np.arctan(ke * crosstrack_error / v)
            steer_expect = min(1.22, steer_expect)
            steer_output = max(-1.22, steer_expect)
